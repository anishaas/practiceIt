...
public class IntTree {
    ...
    public IntTree(IntTreeNode overallRoot, boolean copy) {
        if (copy) {
            this.setOverallRoot(deepCopy(overallRoot));
        } else {
            // just use this one
            this.setOverallRoot(overallRoot);
        }
    }
    ...
    public IntTree(int height) {
        ...
        setOverallRoot(randomTree(height));
    }
    ...
    public IntTree(String s) {
        setOverallRoot(fromString(new StringBuilder(s.toLowerCase().trim())));
    }
    ...
    public boolean equals(Object o) {
        ...
        if (o instanceof IntTree) {
            ...
            return this.getOverallRoot() == other.getOverallRoot() || equals(this.getOverallRoot(), other.getOverallRoot());
        } else {
            return false;
        }
    }
    ...
    public void printStructure() {
        IntTreeNode.clearCycleData();
        printStructure(getOverallRoot(), 0);
    }
    ...
    public String toString() {
        ...
        String result = toString(getOverallRoot(), 0, 0, size, height, widest);
        
        if (getOverallRoot() == null) {
            result = "overallRoot\n   null";
        } else {
            String firstLine = "";
            int nodesLeft = __getSize(getOverallRoot().left);
            if (getOverallRoot().left != null && (getOverallRoot().cycle() || getOverallRoot().left.cycle())) {
				// nodesLeft = 0;
			}
    ...
    private int __getHeight() {
		IntTreeNode.clearCycleData();
		return __getHeight(getOverallRoot());
	}
    ...
    private int __getSize() {
		IntTreeNode.clearCycleData();
		return __getSize(getOverallRoot());
	}
    ...
    private int __getWidest() {
		IntTreeNode.clearCycleData();
		return __getWidest(getOverallRoot());
	}
    ...
    public String toBracketedString() {
        return toBracketedString(getOverallRoot());
    }
